...
export default function Page(){
 ...
 {tab==='Dashboard'&&shell(<div className="grid"><div className="grid cards"><Metric title="Total invoices" v={summary.total}/><Metric title="Pending" v={summary.pending}/><Metric title="Paid" v={summary.paid}/><Metric title="Need review" v={summary.alerts}/></div><div className="card"><h3>Casper Wallet Access</h3><p>Status: <b>{walletStatus}</b></p><p className="muted">{walletMessage}</p>{current&&<p>Whitelisted role: <b>{current.role}</b> — {current.name}</p>}
  <div className="actions"><button className="btn gray" onClick={inspectCasperWalletApi}>Inspect Wallet API</button><button className="btn gray" onClick={testWalletSignature} disabled={!current}>Test Wallet Signature</button><button className="btn gray" onClick={testDeploySignature} disabled={!current}>Test Deploy Signature</button></div>
  {walletApiInfo.length>0&&<div className="notice"><b>Wallet API methods:</b><br/><span className="muted">{walletApiInfo.join(', ')}</span></div>}
  {signatureTest&&<div className="ok"><b>Signature test:</b><br/>{signatureTest}</div>}
  {deploySignatureTest&&<div className="ok"><b>Deploy signature test:</b><br/>{deploySignatureTest}</div>}
  {deploySubmitTest&&<div className="ok"><b>Deploy submit test:</b><br/>{deploySubmitTest}</div>}
